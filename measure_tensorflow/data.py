...
def testGenerator(test_path, num_image=112, target_size=(256, 256), flag_multi_class=False, as_gray=True):
...
def saveResult(test_path, save_path, npyfile, flag_multi_class=False, num_class=2):
    if not os.path.exists(save_path):
        os.mkdir(save_path)
    testImgNames = os.listdir(test_path)
    for i, item in enumerate(npyfile):
        img = labelVisualize(num_class, COLOR_DICT,
                             item) if flag_multi_class else item[:, :, 0]
        io.imsave(os.path.join(save_path, testImgNames[i]), img)
